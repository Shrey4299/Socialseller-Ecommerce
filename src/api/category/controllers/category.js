...
exports.getProducts = async (req, res) => {
    try {
        ...
        const { id } = req.params
        console.log(id)
        const category = await sequelize.models.Category.findByPk(id, { include: ["thumbnail"] });
        ...
        if (category) {
            ...
            const products = await sequelize.models.Product.findAndCountAll({
                distinct: true,
                include: [{ model: sequelize.models.Category, as: "category", where: { id } }],
...
exports.update = async (req, res) => {
    try {
        ...
        if (getcategory) {
            const category = await sequelize.models.Category.update(req.body, { where: { id } });
            if (body.hasOwnProperty("thumbnail")) {
                await sequelize.models.Media.update({ CategoryId: id }, { where: { id: body.thumbnail } })
            }
