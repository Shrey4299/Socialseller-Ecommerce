...
exports.create = async (req, res) => {
  try {
    ...
    if (getStoreSetting.length !== 0) {
      const updateStoreSetting = await sequelize.models.Store_setting.update(
        req.body,
        {
    ...
    console.error(error);
    return res.status(500).send({ error: "Failed to create a store setting" });
  }
};
...
exports.find = async (req, res) => {
  try {
    ...
    return res.status(200).send(storeSetting);
  } catch (error) {
    console.error(error);
    return res.status(500).send({ error: "Failed to fetch store setting" });
  }
};
